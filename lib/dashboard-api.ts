...
export async function uploadDocuments(documents: DocumentToUpload[]): Promise<unknown> {
  ...
  const body = {
    documents: documents.map((d) => ({ type: d.type, content: d.content, filename: d.filename })),
...
}): Promise<{ nodes: unknown[]; overallSummary: string; totalPages: number; title: string }> {
...
export async function getRecentDocuments(limit: number = 5): Promise<SearchResult[]> {
...
// Get document by ID
export async function getDocument(id: string): Promise<Record<string, unknown>> {

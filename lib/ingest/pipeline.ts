...
export function buildManagerFocusedPrompt(meta?: { department?: string; documentType?: string }): string {
...
export async function processDocumentWithAI(
  ...
  try {
    ...
    return { nodes, fullSummary: parsed.overallSummary || 'Document processed successfully' };
  } catch {
...
export async function createLinkedStructure(nodes: Partial<PipelineNode>[]): Promise<PipelineNode[]> {
...
export function heuristicChunkNodes(text: string, opts?: { per?: number }): Partial<PipelineNode>[] {
...
export async function reprocessFromRaw(raw: { type: string; content: string; text?: string }, apiKey: string, meta?: { department?: string; documentType?: string }): Promise<{ nodes: PipelineNode[]; fullSummary: string }>
{
  ...
  if (raw.type === 'pdf') {
    ...
    try {
      ...
      if (agentBad && limited.length > 1) {
        const perPageNodes = limited.map((p) => ({ pageRange: { start: p.index, end: p.index }, content: p.text, summary: p.text.split(/(?<=[.!?])\s+/).slice(0, 2).join(' ').slice(0, 500), keyPoints: [], actionableItems: [] }));
      ...
      if (limited.length > 1) {
        ...
        const aiResult = await processDocumentWithAI(raw.text || '', [], apiKey, meta);
        linkedNodes = await createLinkedStructure(aiResult.nodes);
        overallSummary = aiResult.fullSummary;
      }
    }
  } else {
    const aiResult = await processDocumentWithAI(raw.text || raw.content, [], apiKey, meta);
    linkedNodes = await createLinkedStructure(aiResult.nodes);
    overallSummary = aiResult.fullSummary;
    if (linkedNodes.length <= 1 && ((raw.text || raw.content)?.length || 0) > 4000) {
      linkedNodes = await createLinkedStructure(heuristicChunkNodes(raw.text || raw.content, { per: 1800 }));
      overallSummary = (overallSummary || raw.text || raw.content).slice(0, 600);
    }
  }

...
## Current design and verification contract (8 September 2026)
...
The active text-generation entry point is `lib/ai/generate.ts`. When server-only `GEMINI_API_KEY` is configured, chat, ingestion enrichment and translation use Gemini (`GEMINI_MODEL`, default `gemini-2.5-flash`). Neither setting belongs in the mobile bundle. Without the key, the legacy OpenCode client is attempted once; its free tier currently rejects direct application calls (HTTP 403). If generation is unavailable, chat explicitly reports fallback and selects relevant verbatim source sentences. Configure Gemini for working AI synthesis. The legacy `/api/search/vector` route uses lexical relevance. Feedback reprocessing rereads source, rather than applying correction text as an edit.

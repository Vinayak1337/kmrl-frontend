...
export function Select({ value, onValueChange, placeholder, children, className }: RootSelectProps) {
...
export function SelectTrigger({ className, children, ...props }: SelectTriggerProps) {
  ...
  return (
    <button
      ...
      {...props}
    >
      <span className={cn(!ctx.value && !ctx.selectedLabel ? 'text-gray-500' : '')}>{children ?? label}</span>

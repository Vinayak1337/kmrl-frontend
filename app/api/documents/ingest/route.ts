...
async function processDocumentWithAI(
	...
	try {
		...
		const genConfig = {
			...
			// prevent mid-output truncation for long JSON
			maxOutputTokens: 8192 as unknown as never
		};
		...
		for (let attempt = 0; attempt < 4; attempt++) {
			try {
				...
				result = genResult as Awaited<ReturnType<typeof model.generateContent>>;
				break;
			} catch (e: unknown) {
				const err = e as { status?: number; message?: string };
				const overloaded =
					err?.status === 503 ||
					/overloaded|unavailable/i.test(String(err?.message || ''));
				if (overloaded && attempt < 3) {
					...
					const backoff =
						900 * Math.pow(2, attempt) + Math.floor(Math.random() * 250);
					await sleep(backoff);
					continue;
				}
				throw e;
		...
		// Attempt JSON repair if the model returned truncated/extra text
		// Strategy: extract the last top-level JSON object by scanning braces
		...
		try {
			let parsed: ManagerAnalysisJSON | null = null;
			try {
				parsed = JSON.parse(responseText) as ManagerAnalysisJSON;
			} catch {
				const repaired = tryRepairJson(responseText);
				if (repaired) parsed = JSON.parse(repaired) as ManagerAnalysisJSON;
			}
			if (!parsed) throw new Error('parse failed');
			const nodes = (Array.isArray(parsed.nodes) ? parsed.nodes : []).map(
				(node: ManagerNodeJSON, index: number) => {
					const pageRange = node.pageRange || {
						start: index + 1,
						end: index + 1
					};
					const nodeContent = sliceByPageRange(text, pageRange);
					const md = node.summaryMd ? mdToPlain(node.summaryMd) : '';
					const cleanMd = md.trim();
					const autoSummary = autoGenerateNodeSummary(nodeContent);
					const summary = (
						cleanMd && cleanMd.length >= 60 ? cleanMd : autoSummary
					)
						.replace(/^\s*\d+\s*$/m, '')
						.trim();

					// Use AI-generated summary if substantial, otherwise auto-generate from this page's content
					const finalSummary = isBadAiSummary(summary, nodeContent)
						? autoGenerateSummary(nodeContent)
						: summary;

					return {
						pageRange,
						content: nodeContent,
						summary: finalSummary, // Use the refined summary

...
async function resolveActorId(sub: string): Promise<string> {
	if (/^[0-9a-fA-F]{24}$/.test(sub)) return sub;
	const admin = await prisma.user.findFirst({ select: { id: true } });
	return admin?.id || '000000000000000000000001';
}
...
export async function PUT(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
	...
	if (Array.isArray(body.grants)) {
		const grants = body.grants
			...
			.filter(g => g.dept && g.type && g.actions.length) as Array<{
			dept: string;
			type: string;
			actions: string[];
		}>;
	...
	try {
		...
		const actorId = await resolveActorId(session.sub);
		...
		return NextResponse.json({ id: updated.id, ok: true }, { status: 200 });
	} catch (err) {

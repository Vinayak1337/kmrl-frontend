...
import { SUPPORTED_LANGUAGES, LANGUAGE_CODE_MAP } from '@/lib/languages';
...
export default function DocumentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="desk-page document-page">
    ...
    <Modal open={translate} onClose={() => setTranslate(false)} title="Translate content" description={(tab === 'sections' || split) && activeSection ? activeSection.title : 'Document summary and key points'} busy={translating} wide={!!translation}><form className="form-stack translation-form" onSubmit={event => { event.preventDefault(); void handleTranslate(); }}><label>Language<select value={language} onChange={event => { setLanguage(event.target.value); setTranslation(null); }} disabled={translating}>{SUPPORTED_LANGUAGES.map(l => <option key={l.name} value={l.name}>{l.name} ({l.nativeName})</option>)}</select></label><button className="button button-primary" type="submit" disabled={translating}>{translating && <Loader2 size={15} className="animate-spin" />}{translating ? 'Translating…' : 'Translate'}</button></form>{translationError && <p className="notice error" role="alert">{translationError}</p>}{translation && <div role="status" className="translation-result" lang={LANGUAGE_CODE_MAP[translation.language] || 'en'}><h3>Translation · {translation.language}</h3><Markdown>{translation.summary}</Markdown><Points items={translation.keyPoints} />{translation.actionableItems.length > 0 && <Points items={translation.actionableItems} />}</div>}</Modal>

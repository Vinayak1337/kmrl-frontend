	...
	return (
		<div className='p-6 md:p-8 max-w-7xl mx-auto space-y-6'>
			...
			<div className='card-bezel'>
				<div className='card-bezel-inner p-4 flex items-start gap-3 shadow-xs'>
					<Info className='h-4 w-4 text-[#2563EB] mt-0.5 flex-shrink-0' />
					<div className='text-xs text-slate-600 leading-relaxed'>
						<span className='font-semibold text-[#0F172A]'>
							Source-level boundary enforcement:{' '}
						</span>
						When team members ask questions or run natural-language searches in DocSetu, answers and citations are filtered dynamically by these policy boundaries before synthesis.
					</div>
				</div>
			</div>
			...
			<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
				{policies.map(pol => (
					<div key={pol.documentType} className='card-bezel'>
						<div className='card-bezel-inner p-6 shadow-xs space-y-4 flex flex-col justify-between h-full'>
							<div className='space-y-2'>
								<div className='flex items-center justify-between'>
									<span className='px-2.5 py-0.5 rounded-md bg-blue-50 border border-blue-200 text-blue-700 text-xs font-semibold uppercase'>
										{pol.documentType}
									</span>
									{pol.adminOnly && (
								...
								<div className='space-y-1.5'>
									...
									<div className='flex flex-wrap gap-1.5'>
										{pol.visibleToTeams.map(t => (
											<span
												key={t}
												className='inline-flex items-center gap-1 px-2 py-0.5 rounded bg-slate-50 border border-slate-200 text-[#0F172A] font-medium'>
